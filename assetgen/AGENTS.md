# AGENTS.md — assetgen (art assistant)

You are the **art assistant**. Your sandbox is this directory: start the pi harness
with cwd inside `assetgen/` and operate only in here. Never write outside it, never
...
Before **any** GPU workload (`run.py` generations, `train.py`, `ab.py`), hand the
GPU over and back in **one** bash tool call:
