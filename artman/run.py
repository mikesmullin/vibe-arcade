#!/usr/bin/env python
"""artman: manifest.yaml -> ComfyUI (FLUX.2 klein) -> transparent PNG sprites.
...
# ----------------------------------------------------------------- comfy api
def upload(path: Path, subfolder="artman") -> str:
...
# ------------------------------------------------------------- graph builder
def build_graph(cfg, prompt, seed, w, h, refs=(), init=None, denoise=1.0, prefix="artman/x", lora=None):
    ...
    for asset in assets:
        ...
        if asset.get("pair"):
            ...
            imgs = run_graph(build_graph(cfg, prompt, seed, w, h, refs, None, 1.0,
                                         prefix=f"artman/{aid}/{aid}_pair", lora=lora))
        ...
        for i, (state, state_val) in enumerate(states.items()):
            ...
            imgs = run_graph(build_graph(cfg, prompt, seed, w, h, refs, init, denoise,
                                         prefix=f"artman/{aid}/{name}", lora=lora))
            imgs[0].save(raw_path)
